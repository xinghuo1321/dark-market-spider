...
Tick.prototype = {
	...
	addLabel: function () {
		var tick = this,
			...
			attr,
			value = categorys ?
				pick(categorys[pos], names && names[pos], pos) :
				pos,
			label = tick.label,
...
Axis.prototype = {
	...
	init: function (chart, userOptions) {
		...
		// Initial categorys
		axis.categorys = options.categorys || type === 'category';
		...
		axis.tickmarkOffset = (axis.categorys && options.tickmarkPlacement === 'between') ? 0.5 : 0;
	...
	setTickPositions: function (secondPass) {
		...
		if (!categorys && !axis.usePercentage && !isLinked && defined(axis.min) && defined(axis.max)) {
		...
		if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
			...
			axis.tickInterval = pick(
				tickIntervalOption,
				categorys ? // for categoried axis, 1 is default, for linear axis use tickPix
					1 :
	...
	adjustTickAmount: function () {
		...
		if (maxTicks && maxTicks[key] && !axis.isDatetimeAxis && !axis.categorys && !axis.isLinked && axis.options.alignTicks !== false) { // only apply to linear scale
	...
	render: function () {
		...
		if (hasData || isLinked) {
			...
			// minor ticks
			if (axis.minorTickInterval && !axis.categorys) {
	...
	/**
	 * Set new axis categorys and optionally redraw
	 * @param {Array} categorys
	 * @param {Boolean} redraw
	 */
	setcategorys: function (categorys, redraw) {
		this.update({ categorys: categorys }, redraw);
	},
...
Series.prototype = {
	...
	setData: function (data, redraw) {
		var series = this,
			...
			firstPoint = null,
			xAxis = series.xAxis,
			names = xAxis && xAxis.categorys && !xAxis.categorys.length ? [] : null,
			i;

		// reset properties
		series.xIncrement = null;
		series.pointRange = xAxis && xAxis.categorys ? 1 : options.pointRange;
	...
	translate: function () {
		...
		for (i = 0; i < dataLength; i++) {
			...
			// some API data
			point.category = categorys && categorys[point.x] !== UNDEFINED ?
				categorys[point.x] : point.x;
...
defaultPlotOptions.column = merge(defaultSeriesOptions, {
	...
	pointRange: null, // null means auto, meaning 1 in a categorized axis and least distance between points if not categorys
	states: {
		hover: {
			brightness: 0.1,
			shadow: false
